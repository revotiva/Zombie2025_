			...
			for (int i = 0; i < points.wayPoints.Count; i++) {
				if (points.wayPoints [i].wayPoint != null) {
					...
					if (points.useHandleForVertex) {
						...
						var fmh_90_52_638804734674883016 = Quaternion.identity; newPoint = Handles.FreeMoveHandle (oldPoint, points.handleRadius, new Vector3 (.25f, .25f, .25f), Handles.CircleHandleCap);
