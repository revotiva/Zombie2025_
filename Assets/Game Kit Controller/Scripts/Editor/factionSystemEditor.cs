...
public class factionSystemEditor : Editor
{
            ...
            for (int i = 0; i < list.arraySize; i++) {
                ...
                if (i < list.arraySize && i >= 0) {
                    ...
                    if (list.GetArrayElementAtIndex (i).isExpanded) {
                        expanded = true;
                        showFactionListElement (list.GetArrayElementAtIndex (i), i);
                    }
    ...
    void showFactionListElement (SerializedProperty list, int factionIndex)
    {
        ...
        showRelationWithOtherFactions (list.FindPropertyRelative ("relationWithFactions"), factionIndex);
        GUILayout.EndVertical ();
    ...
    void showRelationWithOtherFactions (SerializedProperty list, int factionIndex)
    {
        GUILayout.BeginVertical ();
        ...
        if (list.isExpanded) {
            ...
            GUILayout.BeginVertical ();
            if (GUILayout.Button ("Set All Relations as Friend")) {
                if (!Application.isPlaying) {
                    manager.setAllRelationsAsFriendOrEnemyOnFaction (factionIndex, 0);
                }
            }
            if (GUILayout.Button ("Set All Relations as Enemy")) {
                if (!Application.isPlaying) {
                    manager.setAllRelationsAsFriendOrEnemyOnFaction (factionIndex, 1);
                }
            }
            if (GUILayout.Button ("Set All Relations as Neutral")) {
                if (!Application.isPlaying) {
                    manager.setAllRelationsAsFriendOrEnemyOnFaction (factionIndex, 2);
                }
            }
            GUILayout.EndVertical ();
