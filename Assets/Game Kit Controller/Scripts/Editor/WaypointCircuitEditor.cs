			...
			for (int i = 0; i < manager.waypointList.Count; i++) {
				if (manager.waypointList [i] != null) {
					...
					if (manager.useHandleForVertex) {
						...
						var fmh_69_52_638804734674868031 = Quaternion.identity; newPoint = Handles.FreeMoveHandle (oldPoint, manager.handleRadius, new Vector3 (.25f, .25f, .25f), Handles.CircleHandleCap);
