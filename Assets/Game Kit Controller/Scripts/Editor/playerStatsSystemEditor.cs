...
public class playerStatsSystemEditor : Editor
{
	...
	void showStatInfoListElement (SerializedProperty list)
	{
        ...
        if (GUILayout.Button ("Initialize Stat Value")) {
            manager.initializeStatValueFromEditor (list.FindPropertyRelative ("Name").stringValue, true);
        }
	...
	void showStatInfoList (SerializedProperty list)
	{
		...
		if (list.isExpanded) {
            ...
            if (GUILayout.Button ("Initialize All Stat Values")) {
				manager.initializeAllStatValuesFromEditor ();
            }
