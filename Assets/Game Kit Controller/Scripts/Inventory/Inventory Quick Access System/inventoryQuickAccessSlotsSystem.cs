...
public class inventoryQuickAccessSlotsSystem : MonoBehaviour
{
    ...
    public void initializeQuickAccessSlots ()
    {
        for (int i = 0; i < numberQuickAccessSlots; i++) {
            ...
            if (hideEmptySlots) {
                if (currentQuickAccessSlotInfo.slot.activeSelf) {
                    currentQuickAccessSlotInfo.slot.SetActive (false);
                }
            }
        }
    ...
    public void enableOrDisableAllQuickAccessSlots (bool state)
    {
        if (hideEmptySlots) {
            for (int j = 0; j < quickAccessSlotInfoListCount; j++) {
                ...
                if (state) {
                    if (currentSlotInfo.slot.activeSelf != state) {
                        currentSlotInfo.slot.SetActive (state);
                    }
                } else {
    ...
    {
        for (int j = 0; j < quickAccessSlotInfoListCount; j++) {
            if (currentQuickAccessList [j].slotActive && currentSlotIndex == j &&
                currentQuickAccessList [j].inventoryInfoAssigned.isWeapon) {
    ...
    public void updateQuickAccessSlotInfo (int slotIndex, inventoryInfo currentSlotToMove, inventoryQuickAccessSlotElement.quickAccessSlotInfo slotToUnEquip, playerWeaponSystem secondaryWeaponToEquip)
    {
        ...
        if (slotIndex > -1 && slotIndex < quickAccessSlotInfoListCount && currentSlotToMove != null) {
            ...
            if (currentSlotInfo != null) {
                ...
                if (hideEmptySlots && !inventoryOpened) {
                    if (!currentSlotInfo.slot.activeSelf) {
                        currentSlotInfo.slot.SetActive (true);
                    }
                }
            }
        }
        ...
        if (!slotFound && slotToUnEquip != null) {
            if (activatingDualWeaponSlot) {
                ...
                if (hideEmptySlots && !inventoryOpened) {
                    if (slotToUnEquip.slot.activeSelf) {
                        slotToUnEquip.slot.SetActive (false);
                    }
                }
            }
        }
    ...
    public void setOpenOrCloseInventoryMenuState (bool state)
    {
        ...
        enableOrDisableAllQuickAccessSlots (state);
    }
