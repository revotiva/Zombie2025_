...
public class weaponAttachmentSystem : MonoBehaviour
{
        ...
        for (int i = 0; i < attachmentInfoList.Count; i++) {
            ...
            if (currentAttachmentPlace.attachmentPlaceEnabled) {
                ...
                if (useOffsetPanels) {
                    ...
                    Vector3 linePosition = curentAttachmentIcon.attachmentLineTransform.GetComponent<RectTransform> ().anchoredPosition;

                    Vector3 pointPosition = curentAttachmentIcon.attachmentPointTransform.GetComponent<RectTransform> ().anchoredPosition;

                    if (linePosition == Vector3.zero) {
                        linePosition = Vector3.one;
                    }

                    if (pointPosition == Vector3.zero) {
                        pointPosition = -Vector3.one;
                    }

                    float distance = GKC_Utils.distance (linePosition, pointPosition);
    ...
    public void openOrCloseWeaponAttachmentEditor (bool state)
    {
        ...
        weaponSystem.setProjectileInfoAssignedValue (false);
    }
    ...
    bool attachmentInitialized;

    bool initializationInProcess;

    bool initializationInStartCalled;

    float lastTimeInitializationCalled;

    public void checkIfAttachmentSystemInitialized ()
    {
        if (attachmentInitialized) {
            return;
        }

        if (initializationInProcess || !initializationInStartCalled) {
            if (Time.time < lastTimeInitializationCalled + 1) {
                return;
            }
        }
        ...
        for (int i = 0; i < attachmentInfoList.Count; i++) {
            ...
            if (currentAttachmentPlace.attachmentPlaceEnabled) {
                for (int j = 0; j < currentAttachmentPlace.attachmentPlaceInfoList.Count; j++) {
                    ...
                    if (currentAttachmentInfo.attachmentEnabled) {
                        if (currentAttachmentInfo.attachmentSlotManager.slotButton ==
                            pressedButton && currentAttachmentInfo.currentlyActive) {

                            attachmentPreviouslyActive = true;
                        } else if (currentAttachmentPlace.attachmentIconManager.notAttachmentButton.slotButton ==
                            pressedButton && currentAttachmentPlace.currentAttachmentSelectedIndex == 0) {
