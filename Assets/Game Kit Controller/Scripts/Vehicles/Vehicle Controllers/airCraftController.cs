...
public class airCraftController : vehicleController
{
	...
	public override void Start ()
	{
		...
		if (realisticMovements) {
			mainRigidbody.linearDamping = realisticSettings.initialDrag;
			mainRigidbody.angularDamping = realisticSettings.initialAngularDrag;
		} else {
			mainRigidbody.linearDamping = nonRealisticSettings.initialDrag;
			mainRigidbody.angularDamping = nonRealisticSettings.initialAngularDrag;
		}

		originalDrag = mainRigidbody.linearDamping;
		originalAngularDrag = mainRigidbody.angularDamping;
...
//					}
		...
		if (driving) {
			...
			if (enginesOn) {
				...
				if (realisticMovements) {
					...
					float extraDrag = mainRigidbody.linearVelocity.magnitude * dragIncreaseFactor;

					// Air brakes work by directly modifying drag. This part is actually pretty realistic!
					mainRigidbody.linearDamping = (brakeActive ? (originalDrag + extraDrag) * brakesForce : originalDrag + extraDrag);

					// Forward speed affects angular drag - at high forward speed, it's much harder for the plane to spin
					mainRigidbody.angularDamping = originalAngularDrag + ForwardSpeed / (mainRigidbody.mass / 10);
					...
					if (mainRigidbody.linearVelocity.magnitude > 0) {
						...
						m_AeroFactor = Vector3.Dot (transform.forward, mainRigidbody.linearVelocity.normalized);
						...
						Vector3 newVelocity = Vector3.Lerp (mainRigidbody.linearVelocity, ForwardSpeed * transform.forward, m_AeroFactor * ForwardSpeed * aeroDynamicForce * currentDeltatime);
						mainRigidbody.linearVelocity = newVelocity;

						if (playerMovingOn3dWorld) {
							// also rotate the plane towards the direction of movement - this should be a very small effect, but means the plane ends up
							// pointing downwards in a stall
							mainRigidbody.rotation = Quaternion.Slerp (mainRigidbody.rotation, Quaternion.LookRotation (mainRigidbody.linearVelocity, transform.up), aeroDynamicForce * currentDeltatime);
						}
					}
					...
					Vector3 liftDirection = Vector3.Cross (mainRigidbody.linearVelocity, transform.right).normalized;
					...
					currentVelocity = mainRigidbody.linearVelocity.magnitude;
					...
					mainRigidbody.linearVelocity = airCraftVelocity;

					EnginePower = Mathf.Lerp (EnginePower, mainRigidbody.linearVelocity.magnitude / 10, currentDeltatime * 5);
				}
			} 
		}
	...
	{
		while (mainRigidbody.linearVelocity.magnitude > 0.1f) {
			mainRigidbody.linearVelocity = Vector3.Lerp (mainRigidbody.linearVelocity, Vector3.zero, Time.deltaTime * 2);
		}
